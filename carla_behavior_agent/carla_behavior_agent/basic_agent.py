...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None, grp_inst=None):
        ...
        self._stop_map = {} # onepiece : dizionario che ha come chiavi gli id degli stop e come valore i waypoints degli stessi 
    ...
    def _pedestrian_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        ############### 
        # police_vehicles = [vehicle for vehicle in vehicle_list if "police" in vehicle.type_id.lower()]
        # police = None
        # if len(police_vehicles) > 0:
        #     police = police_vehicles[0]
        ##############
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                ...
                if target_wpt.road_id != ego_wpt.road_id or target_wpt.lane_id != ego_wpt.lane_id  + lane_offset:
                    ...
                    if target_wpt.road_id != next_wpt.road_id or target_wpt.lane_id != next_wpt.lane_id  + lane_offset:
                        ...
                        target_location_wp = self._map.get_waypoint(target_location)
                        if  target_location_wp.road_id != next_wpt.road_id or target_location_wp.lane_id != next_wpt.lane_id  + lane_offset:
                            continue
    ...
    def _affected_by_stop(self, stops_list=None, max_distance=None): # onepiece : questo metodo viene richiamato sempre
        """
        ...
        if not stops_list:
            stops_list = self._world.get_actors().filter("traffic.stop")
        ...
        for stop in stops_list:
            # Aggiornamento della stop_map (dizionario id_stop : trigger_wp)
            if stop.id in self._stop_map:   # Se il segnale è già presente ...
                trigger_wp = self._stop_map[stop.id]  # aggiunge il trigger wp
            else:   # Se il segnale non era ancora presente...
                trigger_location = get_trafficlight_trigger_location(stop)
                trigger_wp = self._map.get_waypoint(trigger_location)    # aggiunge il trigger_wp
                self._stop_map[stop.id] = trigger_wp   # Lo aggiunge ANCHE al dizionario 

            # Se è troppo lontano continuiamo 
            if trigger_wp.transform.location.distance(ego_vehicle_location) > max_distance:
                continue

            # Se non è sulla stessa strada continuiamo 
            if trigger_wp.road_id != ego_vehicle_waypoint.road_id:
                continue

            #FIXME: Aggiungere ulteriori controlli sullto stop, per es. deve stare sulla stessa corsia (cred), deve essere rivolto verso di noi, ...
    ...
    def set_destination(self, end_location, start_location=None):
        ...
        if None in route_trace:
            print("set_destination c'è un None")
        #print("lunghezza plan:",len(route_trace))
        self._local_planner.set_global_plan(route_trace, clean_queue=clean_queue)

    def set_global_plan(self, plan, stop_waypoint_creation=True, clean_queue=True, global_plan_world_coord = None):
        """
        ...
        for target_vehicle in vehicle_list:
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                ...
                if target_wpt.road_id != ego_wpt.road_id or target_wpt.lane_id != ego_wpt.lane_id  + lane_offset:
                    ...
                    if target_wpt.road_id != next_wpt.road_id or target_wpt.lane_id != next_wpt.lane_id  + lane_offset:
                        ######
                        # if target_vehicle == police:
                            # print("Police 2", target_wpt.road_id, next_wpt.road_id, target_wpt.lane_id, next_wpt.lane_id  + lane_offset, lane_offset)
                        ###### 
                        target_location = target_vehicle.get_location()
                        target_location_wp = self._map.get_waypoint(target_location)
                        if  target_location_wp.road_id != next_wpt.road_id or target_location_wp.lane_id != next_wpt.lane_id  + lane_offset:
                    ...
                    # if target_wpt.road_id != next_wpt.road_id:
                    #     ######
                    #     if target_vehicle == police:
                    #         print("Police 2", target_wpt.road_id, next_wpt.road_id, target_wpt.lane_id, next_wpt.lane_id  + lane_offset, lane_offset)
                    #     ###### 
    ...
    def overtake_in_meter(self, direction="left", distance_same_lane=0, distance_other_lane=0, lane_change_distance=5):
        """
        Changes the path so that the vehicle performs a lane change.
        Use 'direction' to specify either a 'left' or 'right' lane change,
        and the other 3 fine tune the maneuver
        ...
        if(speed < 0.1):
            speed=5
        print(distance_same_lane, distance_other_lane, lane_change_distance)
        print("speed:",speed)
        path = self._generate_lane_change_path_for_overtake(
        ...
        plan = self._update_global_plan(path)

            
        self.set_global_plan(plan)


    def _generate_lane_change_path_for_overtake(self, waypoint, direction='left', distance_same_lane=0, 
        distance_other_lane=3, lane_change_distance=2, 
        check=True, lane_changes=1, step_distance=2):
        """
        This methods generates a path that results in a lane change.
        Use the different distances to fine-tune the maneuver.
        ...
        # # Same lane
        plan = self._update_same_lane(plan, distance_same_lane, step_distance)

        print("SAME LANE: ", len(plan))
        ...
        # Lane change
        plan = self._update_lane_change(plan, lane_changes, lane_change_distance, direction, option, check)


        print("lane_change LANE: ", len(plan))

        # Other lane
        plan = self._update_other_lane(plan, distance_other_lane, step_distance)
        print("other_lane LANE: ", len(plan))
        ...
        # Lane change
        plan = self._update_lane_change(plan, lane_changes, lane_change_distance, direction, option, check, return_to_lane=True)

        # Other lane
        plan = self._update_other_lane(plan, distance_other_lane, step_distance, return_to_lane=True)
    ...
    def begin_overtake_in_meter(self, direction="left", distance_same_lane=0,  distance_other_lane=0, lane_change_distance=5):
        """
        Changes the path so that the vehicle performs a lane change.
        Use 'direction' to specify either a 'left' or 'right' lane change,
        and the other 3 fine tune the maneuver
        ...
        print(distance_same_lane, distance_other_lane, lane_change_distance)
        print("speed:",speed)
        path = self._generate_lane_change_path_for_begin_overtake(
            waypoint=self._map.get_waypoint(self._vehicle.get_location()),
            direction=direction,
            distance_same_lane=distance_same_lane,
            distance_other_lane=distance_other_lane,
    ...
    def _generate_lane_change_path_for_begin_overtake(self, waypoint, direction='left', distance_same_lane=0, 
        distance_other_lane=3, lane_change_distance=2, 
        check=True, lane_changes=1, step_distance=2):
        """
        This methods generates a path that results in a lane change.
    ...
    def end_overtake_in_meter(self, direction="left", distance_same_lane=4, distance_other_lane=15, lane_change_distance=3):
        """
        Changes the path so that the vehicle performs a lane change.
        Use 'direction' to specify either a 'left' or 'right' lane change,
        and the other 3 fine tune the maneuver
        """
        print(distance_other_lane, lane_change_distance)
        path = self._generate_lane_change_path_for_end_overtake(
            waypoint=self._map.get_waypoint(self._vehicle.get_location()),
            direction=direction,
            distance_same_lane=distance_same_lane,
            distance_other_lane=distance_other_lane,
    ...
    def _generate_lane_change_path_for_end_overtake(self, waypoint, direction='left', distance_same_lane=2, distance_other_lane=3,
        lane_change_distance=2, check=True, lane_changes=1,
        step_distance=2):
        """
        This methods generates a path that results in a lane change.
        Use the different distances to fine-tune the maneuver.
        ...
        # plan = self._update_same_lane(plan, distance_same_lane, step_distance, other_line=False)
    ...
    def _update_global_plan(self, path):
        plan = [(CarlaDataProvider.get_map().get_waypoint(x[0].location),x[1]) for x in self._global_plan_world_coord]

        last_corner_wp = path[-1][0] 
        previous_dist = 99999 
        i=0 
        for i in range(len(plan)): 
            distance = plan[i][0].transform.location.distance(last_corner_wp.transform.location) 
            # print("distanza:",distance) 

            if(distance > previous_dist and distance < 10): 
                plan = plan[i:] 
                break 

            previous_dist = distance 

        print("LA distanza", previous_dist)
        ...
        # for i in range(5):
        #     print("PRIMI PASSI", path[i][0].lane_id)
    ...
    def _update_same_lane(self, plan, distance_same_lane, step_distance, other_line=False):
        # Same lane
        distance = 0
        while distance < distance_same_lane:
            if not other_line:
    ...
    def _update_lane_change(self, plan, lane_changes, lane_change_distance, direction, option, check, return_to_lane=False):
        ...
        while lane_changes_done < lane_changes:
            ...
            if direction == 'left':
                if check and str(next_wp.lane_change) not in ['Left', 'Both']:
                    return []
                side_wp = next_wp.get_left_lane()#.previous(step_distance)[0] #aggiunto previous!!!!!!!
            else:
                if check and str(next_wp.lane_change) not in ['Right', 'Both']:
                    return []
                side_wp = next_wp.get_right_lane()#.previous(step_distance)[0] #aggiunto previous!!!!!!!
    ...
    def _update_other_lane(self, plan, distance_other_lane, step_distance, return_to_lane=False):
        distance = 0
        while distance < distance_other_lane:
            if not return_to_lane:
                next_wps = plan[-1][0].previous(step_distance)
